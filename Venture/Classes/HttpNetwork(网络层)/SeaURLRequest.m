...
- (void)buildPostBody
{
    ...
    if(self.useECStoreSignature)
    {
        ...
        [self.postParams addObject:[SeaHttpRequestParam requestParamWithValue:[NSDate getCurrentTimeWithFormat:@"yyyy-MM-dd kk:mm:ss"] key:@"date"]];
        [self.postParams addObject:[SeaHttpRequestParam requestParamWithValue:@"true" key:@"direct"]];
...
//        NSLog(@"result_signatureParams:%@", signature);
...
- (void)bulidURLEncodedPostBody
{
    ...
    for(SeaHttpRequestParam *param in self.postParams)
    {
        ...
        {
            [postData appendData:
             [[NSString stringWithFormat:@"%@=%@%@", param.key, param.value, i == self.postParams.count - 1 ? @"" : @"&"]

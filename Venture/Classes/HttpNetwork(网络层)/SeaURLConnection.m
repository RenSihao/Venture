...
- (id)initWithDelegate:(id<SeaURLConnectionDelegate>) delegate request:(SeaURLRequest *)request
{
    ...
    if(self)
    {
        ...
        _URL = [request.request.URL absoluteString];
        ...
        _errorCode = SeaHttpErrorCodeNoError;
...
- (void)dealloc
{
   ...
   // NSLog(@"SeaURLConnection dealloc");
}
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error
{
    if(error.code != NSURLErrorCancelled)
    {
        ...
        _errorCode = SeaHttpErrorCodeNotKnow;
        
        switch (error.code)
        {
            case NSURLErrorTimedOut :
                _errorCode = SeaHttpErrorCodeTimeOut;
                break;
            case NSURLErrorCannotFindHost :
                _errorCode = SeaHttpErrorCodeCanNotFound;
                break;
            case NSURLErrorCannotConnectToHost :
                _errorCode = SeaHttpErrorCodeBadNetwork;
                break;
            default:
                break;
        }
...
//        NSLog(@"statusCode %ld", statusCode);
        ...
        if(statusCode < 200 || statusCode > 299)
        {
            ...
            {
                case 400 :
                    _errorCode = SeaHttpErrorCodeErrorParam;
                    break;
                case 404 :
                    _errorCode = SeaHttpErrorCodeCanNotFound;
                    break;
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data
{
    ...
    if(self.downloadTemporayPath)
    {
        ...
        if(self.outputStream.streamError)
        {
            ...
            if([self.delegate respondsToSelector:@selector(connectionDidFail:)])
            {
                _errorCode = SeaHttpErrorCodeWriteOperation;
                [self.delegate connectionDidFail:self];
            }
        }
    }

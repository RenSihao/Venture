...
- (void)layoutSubviews
{
    ...
    frame.size.height = MAX(SeaRefreshControlCriticalPoint, - self.scrollView.contentOffset.y);
...
{
    if(self.scrollView.contentOffset.y <= 0.0f  && [keyPath isEqualToString:SeaDataControlOffset])
    {
        if(self.state != SeaDataControlLoading)
        {
            if(self.scrollView.dragging)
            {
                ...
                else if (self.scrollView.contentOffset.y > - SeaRefreshControlCriticalPoint)
                {
            ...
            else if(self.scrollView.contentOffset.y <= - SeaRefreshControlCriticalPoint || self.state == SeaDataControlReachCirticalPoint)
            {
                if(!self.animating)
                {
                    self.animating = YES;
...
- (void)updatePosition
{
    CGSize size = [_statusLabel.text stringSizeWithFont:_statusLabel.font contraintWith:self.frame.size.width - _indicatorView.width];
    _indicatorView.left = (self.frame.size.width - size.width - _indicatorView.width) / 2.0;
    
    CGRect frame = _indicatorView.frame;
    frame.origin.x = _indicatorView.right + 3.0;
    frame.size.width = self.frame.size.width - _indicatorView.right;

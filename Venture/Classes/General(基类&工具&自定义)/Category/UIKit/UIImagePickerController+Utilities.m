...
+ (BOOL)canUseCamera
{
    ...
    else if ([AVCaptureDevice authorizationStatusForMediaType:AVMediaTypeVideo] == AVAuthorizationStatusDenied)
    {
        ...
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"" message:msg delegate:[UIApplication sharedApplication].delegate cancelButtonTitle:@"确定" otherButtonTitles:@"取消", @"去设置", nil];
        [alertView show];
//        SeaAlertView *alertView = [[SeaAlertView alloc] initWithTitle:msg otherButtonTitles:[NSArray arrayWithObjects:@"取消", @"去设置", nil]];

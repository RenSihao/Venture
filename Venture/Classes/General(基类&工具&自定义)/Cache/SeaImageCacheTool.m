...
- (id)init
{
    ...
    if(self)
    {
        ...
        _cacheQueue = dispatch_queue_create("cacheImage", NULL);
...
//缩略图文件路径
//- (NSString*)thumbnailPathForURL:(NSString*) url size:(CGSize) size
//{
//    NSString *fileName = [SeaFileManager fileNameForURL:url suffix:nil];
//    fileName = [fileName stringByAppendingFormat:@"-w%.f-h%.f.%@", size.width, size.height, SeaImageCacheToolImageJPGType];
//    
//    return [self.cachePath stringByAppendingPathComponent:fileName];
//}
 ...
 */
- (void)cacheImage:(UIImage*) image withURL:(NSString*) url thumbnailSize:(CGSize) size saveToMemory:(BOOL) flag
{
    ...
    if(flag)
    {
        [self cacheImage:image thumbnailSize:size forURL:url];
    }

 ...
 */
typedef void(^SeaNetworkQueueRequestCompletionHandler)(SeaNetworkQueue *queue, NSData *data, SeaHttpErrorCode code, NSString *identifier);
 ...
 *@param identifier 请求标识
 */
- (void)networkQueue:(SeaNetworkQueue*) quque requestDidFailWithError:(SeaHttpErrorCode) error identifier:(NSString*) identifier;
...
/**更新整个队列的进度条
 */
- (void)networkQueue:(SeaNetworkQueue *)quque didUpdateQueueUploadProgress:(float) progress downloadProgress:(float) downlodProgress;
